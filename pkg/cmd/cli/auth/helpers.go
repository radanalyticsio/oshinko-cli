...
func getMatchingClusters(clientConfig rest.Config, kubeconfig clientcmdapi.Config) sets.String {
	ret := sets.String{}
...
func dialToServer(clientConfig rest.Config) error {
	// take a RoundTripper based on the config we already have (TLS, proxies, etc)
	rt, err := rest.TransportFor(&clientConfig)
	if err != nil {
		return err
	}
...
func hasExistingInsecureCluster(clientConfigToTest rest.Config, kubeconfig kclientcmdapi.Config) bool {
