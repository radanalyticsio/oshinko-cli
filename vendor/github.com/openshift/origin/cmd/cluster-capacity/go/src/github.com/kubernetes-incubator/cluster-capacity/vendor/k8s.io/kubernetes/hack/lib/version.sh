...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='78ddc106ab9a1747452c52da67dd27d66d8bbb9c'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'tag: v3.9.0-alpha.3' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'tag: v3.9.0-alpha.3' =~ tag:\ (v[^ ]+) ]]; then
