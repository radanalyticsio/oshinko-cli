...
usage() {
    ...
    echo "  -r REPO       the Git source repo to build the test from (default: https://github.com/radanalyticsio/oshinko-rest)"
...
case "$REQUESTED_TEST" in
    ...
    client)
        ...
        if [ -z "$SOURCE_REPO" ]
        then
            SOURCE_REPO=https://github.com/radanalyticsio/oshinko-rest
        fi
